...
const roads = [
    ...
    "Ernie's House-Grete's House",  "Grete's Housue-Farm",
...
/*
The network of roads in the village forms a "graph".  A graph is a collection of points (places in the village)
with lines between them (roads).
This graph will be thwe world that our robot moves through.

The array of stringgs isn't very easy to work with.  
What we're interested in is the destinations that we can reach from a given place.

Le'ts convert the list of roads to a data structure that, for each place, tells us
...
const raodGraph = buildGraph(roads);

/*
Given an array of edges, 'buildGraph' creates a map object that, for each node,
stores an array of connected nodes.

It uses the 'split' method to go from the road strings, which have the form of "Start-End",
...
Our robot will be moving around the village.  There are parcels in various places,
each addressed to some other place.  The robot picks up parcels when it comes to them
and delivers them when it arrives at their destination.
...
This model tells us where the robot is and where the parcels are.  When the robot has
decided to move somewhere, we need to update the model to reflect the new situation.

If you're thinking in terms of Object-Oriented Programming, your first impulse might
be to start defining objects for the various elements in the world: a class for
the robot, one for a parcel, maybe one for places.  These could then hold properties
that describe their current state, such as a pile of parcels at a location, 
...
automatically mean that it should be an object in hyour program.
Reflexively writing classes for every concept in your application tends to leave
you with a collection of interconnected objects that have their own internal,
...
that define it.  There's a robot's current location and the collection of undelivered
parcels, each of which has a current location and a destination address. That's it.

And while we're at it, let's make it so that we don't "change" this state when the
...
/*
The 'move' method is wehre the action happens.  It first checks whether there is a road
going from the current place to the destination, and if not, it returns the old state
...
they need to be removed from the set of underlivered parcels. 
...
office and the parcel is undelivered.
...
They behave a lot like strings and numbers in that they are who they are and
stay that way, rather than containing different things at different times.
...
writing to its properties is ignoreed.  You could use that to make sure your
objects aren't changed, if you want to be careful.
Freezing does require the computer to do some extra work, and having updates
ignored is just about as likely to confuse someone as having them do the 
wrong thing.  So, I usually prefer to just tell people that a given object
...
I can consider operations on them in isolation - moving to Alice's hose
from a given start state always produces the same new state.
When objects change over time, that adds a whole new dimension of
complexity to this kind of reasoning.

For a small system like this one we are building in this chapter, 
we could handle that bit of extra complexity.  But the most important 
limit on what kind of systems we build is how much we can understand.
Anything that makes your code easier to understand, makes it possible
to build a more ambitious system.

Unfortunately, although understanding a system built on persisten data
structures is easier, "designing" one, especially when your programming
language isn't helping, can be a litter harder.
We'll look for opportunities to use persisten data structures in this
book, but we'll also be using changable oens.
...
Becasue we want robots to be able to remember things, so that they can
...
The robot could jusut walk in a random direction every turn.
That means, wich great likelihood, it will eventually run into all parcels
and then also at some point reach the place where they should be delivered.
*/
...
/*
Remember that 'Math.random()' returns a number between zero and 1,
but always below one.
Multiplying such a number by the length of an array and then applying
...
argument (remember that JS functions can be called with extra arguments
...
are addressed to.  For this reason, the 'do' loop keeps picking new places
...
/*
It take teh robot a lot of turns to deliver the parcels because it isn't
...
An easy improvement would be to take a hint form the way real-world mail
delivery works.  
If we find a route that passes all places in the village, the robot could 
run that route twice, at which point it is guaranteed to be done.  
*/
...
The robot keeps the rest of its route in its memory and drops the 
...
// This robot is a lot faster.  It'll take a max of 26 turns, but usually less.
...
To do that, it has to be able to dliberately move toward a given parcel or toward
the location where a parcel has to be delivered.  Doing that, even when the goal
is more than one move away, will require some kind of route-finding function.
...
can't directly compute the solution the way we could for something like 2+2.
Instead, we have to keep creating potential solutions until we find on that works.

The number of possible routes through a graph is infinite.  But when searching
for a route from A to B, we are interested only in the ones that start at A.
...
In face, we are mostly interested in the "shortest" route.
So we want to make sure we look at short routes before we look at longer ones.
A good approach would be to "grow" routes from the starting point, exploring
every reachable place that hasn't been visited yet, until a route reaches
the goal.  That way, we'll only explore routes that are potentially interesting,
and we'll find the shortest route (or one of the shortest routes, if there are
more than one) to the goal.
*/
...
first have to be explored first.
We can't immediately explore a place as soon as we reach it because that would
mean places reached "from there" would also be explored immediately, and so on,
...
should be explored next, along with the route that got us there.
...
we've found either a longer route to that place or one precisely as long as the
existing one, and we don't need to explore it.

You can visually imagine this as a web of known routes crawling out from the
start location, growing evenly on all sides (but never tangling back into itslef).
As soon as the first thread reach the goal location, that thread is traced back
to the start, giving us our route.

Our code doesn't handle the situation where there are no more work items
on the work list becasue we know that our graph is "connected", meaning
that every location can be reached from all other locations.
...
If the parcel "has" been picked up, it still needs to be delivered, so the robot
creates a route toward the delivery address instead.
...
Maybe one robot just happened to get easier tasks or the other kind of tasks that 
it is good at, whereas the other didn't.
...
Can you write a robot that finishes a task faster than "goalOrientedRobot"?
